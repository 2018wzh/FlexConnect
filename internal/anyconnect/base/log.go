...
type baseStdLoggerWriter struct{}

func (w baseStdLoggerWriter) Write(p []byte) (n int, err error) {
	msg := strings.TrimSpace(string(p))
	if msg == "" {
		return len(p), nil
	}
	logging.WithComponent("anyconnect").Info(msg)
	return len(p), nil
}
...
func InitLog() {
	...
	logging.Init(baseWriter, logging.ParseLevel(Cfg.LogLevel), true)
...
func output(l int, s ...interface{}) {
	message := strings.TrimSuffix(fmt.Sprintln(s...), "\n")
	switch l {
	case _Debug:
		logging.WithComponent("anyconnect").Debug(message)
	case _Info:
		logging.WithComponent("anyconnect").Info(message)
	case _Warn:
		logging.WithComponent("anyconnect").Warn(message)
	case _Error:
		logging.WithComponent("anyconnect").Error(message)
	case _Fatal:
		logging.WithComponent("anyconnect").Fatal(message)
	default:
		logging.WithComponent("anyconnect").Info(message)
	}
}
